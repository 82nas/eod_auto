```python
    ...
    # --- 경로 & 상수 ---
    ROOT_PATH_UNCHECKED = pathlib.Path('.').resolve()
    ...
    if not available_backends:
        print("[bold yellow]경고: 사용 가능한 AI 백엔드가 없습니다.[/]")
    ...
    class AIProvider:
        def __init__(self, backend_name: str, config: Dict[str, Any]):
            ...
            try:
                ...
                raise e
        def make_summary(self, task: str, ctx: str, arts: List[str]) -> str: return self.backend.make_summary(task, ctx, arts)
        def verify_summary(self, md: str) -> Tuple[bool, str]:
            ...
            return is_ok, msg
        def load_report(self, md: str) -> str: return self.backend.load_report(md)
    ...
    class GitRepo:
        ...
        def list_states(self) -> List[Dict]:
            ...
            items = [];
            try: commits = list(self.repo.iter_commits(max_count=100, first_parent=True, paths=str(STATE_DIR)))
            except Exception: commits = list(self.repo.iter_commits(max_count=100, first_parent=True)) # Fallback
            for c in commits:
                if not c.message.startswith(COMMIT_TAG): continue
                headline = ""; meta_blob = None
                try:
                    if STATE_DIR.name in c.tree:
                        for item in c.tree[STATE_DIR.name].traverse():
                            if isinstance(item, Blob) and item.name.endswith(".meta.json"): meta_blob = item; break
                    if not meta_blob: # Broader search
                         for item in c.tree.traverse():
                              if isinstance(item, Blob) and item.path.startswith(STATE_DIR.name) and item.path.endswith(".meta.json"): meta_blob = item; break
                    if meta_blob: metadata = json.loads(meta_blob.data_stream.read().decode('utf-8')); headline = metadata.get("headline", "")
                except Exception as e: headline = "[메타데이터 오류]"
        ...
        def load_state(self, commit_hash: str) -> str:
            ...
            try:
                if STATE_DIR.name in commit_obj.tree:
                    for item in commit_obj.tree[STATE_DIR.name].traverse():
                ...
                for item in commit_obj.tree.traverse(): # Fallback search
                     if isinstance(item, Blob) and item.path.startswith(STATE_DIR.name) and item.path.endswith(".md"): return item.data_stream.read().decode('utf-8')
                raise RuntimeError(f"커밋 '{commit_hash}' 내 '{STATE_DIR.name}' 폴더에 .md 파일 없음.")
            except KeyError: raise RuntimeError(f"커밋 '{commit_hash}'에 '{STATE_DIR.name}' 폴더 없음.")
    ...
    class Serializer:
        ...
        def save_state(md: str, task: str) -> Tuple[List[pathlib.Path], Optional[pathlib.Path]]:
            ...
            if not safe_task: safe_task="untitled"; base_fn = f"{ts}_{safe_task}"
            STATE_DIR.mkdir(exist_ok=True); ART_DIR.mkdir(exist_ok=True)
            state_f = STATE_DIR / f"{base_fn}.md"; html_f = STATE_DIR / f"{base_fn}.html"; meta_f = STATE_DIR / f"{base_fn}.meta.json"
            ...
            snap_dir = None; checksums = {}; arts = [f for f in ART_DIR.iterdir() if f.is_file()] if ART_DIR.exists() else []
            if arts:
                snap_dir = ART_DIR / f"{base_fn}_artifacts"; snap_dir.mkdir(parents=True,exist_ok=True)
                print(f"[dim]아티팩트 스냅샷 ({len(arts)}개) -> '{snap_dir.relative_to(ROOT_PATH_UNCHECKED)}'[/]")
                for f_art in arts:
                    ...
                    except Exception as copy_e: print(f"[yellow]경고: 아티팩트 복사/해시 실패 ({f_art.name}): {copy_e}[/]")
            headline = "";
    ...
    class Handover:
        def __init__(self, backend_choice: str, current_root: pathlib.Path):
            ...
            global STATE_DIR, ART_DIR # Ensure these are updated based on current_root
            STATE_DIR = self.root_dir / "ai_states"; ART_DIR = self.root_dir / "artifacts"
            try: self.git = GitRepo(self.root_dir)
            except InvalidGitRepositoryError: self.git = None # Allow running outside git for some commands
        ...
        def _ensure_git_ai(self,cmd:str,needs_git:bool,needs_ai:bool):
        ...
        def save(self):
            self._ensure_git_ai("save", True, True)
            try:
                ...
                STATE_DIR.mkdir(exist_ok=True); ART_DIR.mkdir(exist_ok=True) # Ensure dirs exist before listing
                ...
                state_files, snap_dir = Serializer.save_state(md_summary, task)
        ...
        def load(self, latest: bool = False):
            self._ensure_git_ai("load", True, True)
            try:
                states = self.git.list_states();
                ...
                md_content = self.git.load_state(sel_hash)
        ...
        def diff(self, target: str = "HEAD"):
            self._ensure_git_ai("diff", True, False) # Diff needs Git, not AI
            try:
        ...
        def verify_checksums(self, commit_hash: str):
            self._ensure_git_ai("verify", True, False) # Verify needs Git, not AI
            ...
            try:
                ...
                global STATE_DIR; STATE_DIR = self.root_dir / "ai_states"
                meta_blob = None
                search_paths = [STATE_DIR.name] # Search within state_dir first
                # More robust search for meta file
                for item_path_str in [p.path for p in commit.tree.traverse()]:
                    if item_path_str.startswith(STATE_DIR.name) and item_path_str.endswith(".meta.json"):
                        meta_blob = commit.tree[item_path_str]; break
                if not meta_blob: self.ui.error(f"커밋 {commit_hash} 메타파일 없음."); return
    ...
    def main_cli():
        global ROOT_PATH_UNCHECKED, STATE_DIR, ART_DIR

        actual_git_root = None
        try:
            actual_git_root = pathlib.Path(Repo('.', search_parent_directories=True).working_tree_dir)
            ROOT_PATH_UNCHECKED = actual_git_root
            is_git_repo_check = True
        except InvalidGitRepositoryError:
            is_git_repo_check = False

        # Update paths based on actual root
        STATE_DIR = ROOT_PATH_UNCHECKED / "ai_states"
        ART_DIR = ROOT_PATH_UNCHECKED / "artifacts"

        parser = argparse.ArgumentParser(description="AI 기반 프로젝트 인수인계 상태 관리 도구 (v1.1.2)", formatter_class=argparse.RawTextHelpFormatter)
        ...
        parser.add_argument("--backend", default=os.getenv("AI_BACKEND", default_be),
                            choices=backend_choices + ["none"],
                            help=f"AI 백엔드 (기본값: 환경변수 AI_BACKEND 또는 {default_be}). 사용가능: {', '.join(backend_choices) or '없음'}. 'none'으로 AI 비활성화.")
        ...
        # Global dir creation after root is determined
        STATE_DIR.mkdir(exist_ok=True)
        ART_DIR.mkdir(exist_ok=True)

        # Check prerequisites for the chosen command
        chosen_cmd_config = next(c for c in cmd_configs if c[0] == args.command)
        if chosen_cmd_config[2] and not is_git_repo_check: # Git required but not a repo
        ...
        if chosen_cmd_config[3] and (not available_backends or args.backend == "none"):
            msg = "사용 가능한 AI 백엔드가 없습니다." if not available_backends else f"AI 백엔드가 '{args.backend}'로 설정되었습니다."
            UI.error(f"'{args.command}' 명령 실행 불가: {msg} AI 기능이 필요합니다.")
            sys.exit(1)

        print(f"[bold underline]Handover 스크립트 v1.1.2[/]")
        if is_git_repo_check and actual_git_root: print(f"[dim]프로젝트 루트: {actual_git_root}[/]")
        else: print(f"[dim]현재 작업 폴더: {ROOT_PATH_UNCHECKED} (Git 저장소 아님)[/]")

        try:
            # Pass the determined root to Handover instance
