        ...
        try:
            ...
            for name, obj in module.__dict__.items():
                if (isinstance(obj, type) and
                        AIBaseBackend and issubclass(obj, AIBaseBackend) and # AIBaseBackend None 체크 추가
    ...
    def verify_summary(self, md: str) -> Tuple[bool, str]:
        ...
        if is_ok:
            ...
            if len(headers) != len(required_headers_structure):
                ...
                print(f"[DEBUG] Internal Check FAILED: {failure_reason}") # 실패 사유 출력
...
class GitRepo:
    def __init__(self, repo_path: pathlib.Path):
        ...
        try:
            self.repo = Repo(repo_path, search_parent_directories=True) # 상위 디렉토리 검색 추가
            if self.repo.working_dir != str(repo_path.resolve()): # 실제 찾은 경로가 입력 경로와 다르면 알려줌
                 print(f"[dim]Git 저장소 루트 발견: {self.repo.working_dir}[/dim]")
            if self.repo.bare:
                print(f"[yellow]경고: '{self.repo.working_dir}'는 bare 저장소입니다. 작업 디렉토리 분석이 제한될 수 있습니다.[/]")
            ...
            print(f"[red]오류: Git 명령어 실행 실패 ({e.command}). Git 설치 및 PATH 확인 필요.[/]")
    ...
    def save(self, state_paths: List[pathlib.Path], task: str, snapshot_dir: Optional[pathlib.Path]) -> str:
        ...
        for p in state_paths:
            try: paths_to_add_rel.append(p.resolve().relative_to(repo_root).as_posix()) # Posix 경로로 변환
            except ValueError: print(f"[yellow]경고: Git 저장소 외부 경로 추가 시도 무시됨: {p}[/]")
        if snapshot_dir and snapshot_dir.exists() and any(snapshot_dir.iterdir()):
            try: paths_to_add_rel.append(snapshot_dir.resolve().relative_to(repo_root).as_posix())
        ...
        try:
            ...
            if "nothing to commit" in err_str or \
               ...
               "changes not staged for commit" in err_str :
                ...
                commit_hash = self.repo.head.commit.hexsha[:8] + " (변경 없음)" # 현재 HEAD 커밋 해시 사용
            else:
...
class Serializer:
    ...
    def _generate_html(md: str, title: str) -> str:
        ...
        try:
             ...
             escaped_md = "".join(c if c.isalnum() or c in " .,;:!?/\\#$%&'()*+-=<>[]_{}|`~" else f"&#{ord(c)};" for c in md)
    ...
    def save_state(md: str, task: str, current_app_state_dir: pathlib.Path, current_app_art_dir: pathlib.Path, current_app_root: pathlib.Path) -> Tuple[List[pathlib.Path], Optional[pathlib.Path]]:
        ...
        if current_app_art_dir.exists() and current_app_art_dir.is_dir():
            ...
            if arts:
                ...
                except OSError as e: print(f"[yellow]경고: 스냅샷 디렉토리 생성 실패 ({snap_dir}): {e}[/]"); snap_dir=None # 생성 실패 시 스냅샷 진행 불가
...
def main_cli_entry_point():
    ...
    try:
        ...
        if cli_root_path.resolve() == found_root_path.resolve() or cli_root_path in found_root_path.parents:
    ...
    parser = argparse.ArgumentParser(description="AI 기반 프로젝트 인수인계 상태 관리 도구 (v1.1.6 - Debug)", formatter_class=argparse.RawTextHelpFormatter) # 버전명에 Debug 추가
    ...
    print(f"[bold underline]Handover 스크립트 v1.1.6 (Debug Mode)[/]") # 버전명 수정
