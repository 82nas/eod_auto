            ...
            try:
                ...
                if not meta_blob:
                     for item in c.tree.traverse():
                          if isinstance(item, Blob) and item.path.startswith(current_state_dir.name) and item.path.endswith(".meta.json"): meta_blob = item; break
    ...
    def load_state(self, commit_hash: str, current_state_dir: pathlib.Path) -> str: # Pass state_dir
        ...
        try:
            ...
            for item in commit_obj.tree.traverse():
                 if isinstance(item, Blob) and item.path.startswith(current_state_dir.name) and item.path.endswith(".md"): return item.data_stream.read().decode('utf-8')
            raise RuntimeError(f"커밋 '{commit_hash}' 내 '{current_state_dir.name}' 폴더에 .md 파일 없음.")
